...
function die {
    ...
    echo -e "$prefix: "
    echo -e "$prefix: ${bold}You can contact task for further assistance.${reset}"
    echo -e "$prefix: "
    
    echo -e "$prefix: ${bold}URL: https://github.com/go-task/task${reset}"
    echo -e "$prefix: "
...
function config_url {
    ...
    test ! -z "$dnf_version" && {
        params="${params}&dnf_version=${dnf_version}"
    }
    echo "https://dl.cloudsmith.io/public/task/task/config.deb.txt?$params" | sed 's/ /%20/g'
}
...
function import_gpg_key {
    local text="Importing 'task/task' repository GPG keys ..."
    echo_running "$text"

    local gpg_keyring_path="/usr/share/keyrings/task-task-archive-keyring.gpg"
    
    curl -1sLf "https://dl.cloudsmith.io/public/task/task/gpg.046FD1186CA342F0.key" | gpg --dearmor >> $gpg_keyring_path
    ...
    echo_okfail "Checking for apt signed-by key support ..." || {
        mv ${gpg_keyring_path} /etc/apt/trusted.gpg.d/task-task.gpg
        chmod 644 /etc/apt/trusted.gpg.d/task-task.gpg
    }
...
function setup_repository {
    local repo_path="/etc/apt/sources.list.d/task-task.list"
    check_fetch_config

    local text="Installing 'task/task' repository via apt ..."
    echo_running "$text"
...
echo "Executing the  setup script for the 'task/task' repository ..."
echo
