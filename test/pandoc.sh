...
function step_generate_pdf_from_markdown() {
  local SECONDS=0
  local MICROCI_STEP_NAME="Generate PDF from Markdown"
...

  {
    (
      ...
      docker run \
        ...
        --attach stderr \
        --rm \
        --name microci_generate_pdf_from_markdown_$(head -c 8 /proc/sys/kernel/random/uuid) \
  ...
  if microCI_should_pull_docker_image 'intmain/microci_pandoc:latest'; then
    echo 'Updating intmain/microci_pandoc:latest docker image...'
    docker pull intmain/microci_pandoc:latest --quiet
  fi
