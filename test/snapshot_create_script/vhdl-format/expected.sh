...
# ----------------------------------------------------------------------
function step_vhdl_format() {
  local SECONDS=0
  local MICROCI_STEP_NAME="VHDL format"
...

  {
    (
      ...
      docker run \
        ...
        --attach stderr \
        --rm \
        --name microci_vhdl_format_$(head -c 8 /proc/sys/kernel/random/uuid) \
        --network none \
...
if microCI_should_pull_docker_image 'intmain/microci_ghdl:latest'; then
  echo 'Updating intmain/microci_ghdl:latest docker image...'
  docker pull intmain/microci_ghdl:latest --quiet
fi
