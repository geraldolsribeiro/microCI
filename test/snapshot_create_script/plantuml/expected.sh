...
# ----------------------------------------------------------------------
function step_build_diagrams() {
  local SECONDS=0
  local MICROCI_STEP_NAME="Build diagrams"
...

  {
    (
      ...
      docker run \
        ...
        --attach stderr \
        --rm \
        --name microci_build_diagrams_$(head -c 8 /proc/sys/kernel/random/uuid) \
        --network none \
