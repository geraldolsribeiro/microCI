...
# test_all.sh via the Makefile).
#
# Full command chain during CI/local runs:
#   make -C test snapshot_create_script_test
#     -> ./snapshot_create_script/test_all.sh
#       -> ./<plugin>/test.sh
#         -> ./runner_helper.sh <name> <input.yml> <expected.sh>
#
...
if diff --color --unified "$workdir/expected.norm" "$workdir/out.norm"; then
  echo "[create script] PASS  $name"
  status=0
else
  echo "[create script] FAIL  $name"
  status=1
fi

if [[ ${SNAPSHOT_SUMMARY:-0} == 1 ]]; then
  echo "[create script] done   $name"
fi
