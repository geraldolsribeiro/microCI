...

  {
    (
      ...
      docker run \
        ...
        --attach stderr \
        --rm \
        --name microci_build_static_version_of_microci_$(head -c 8 /proc/sys/kernel/random/uuid) \
        --network none \
        --workdir /microci_workspace \
        --volume "${MICROCI_PWD}":"/microci_workspace":rw \
        "intmain/microci_cpp_compiler:latest" \
        /bin/bash -c "cd /microci_workspace \
           && gcc prog.c -o prog_c 2>&1 \
           && g++ prog.cpp -o prog_cpp 2>&1 \
           && ./prog_c 2>&1 \
           && ./prog_cpp 2>&1 \
           && make clean 2>&1 \
           && clang-22 prog.c -o prog_c 2>&1 \
           && clang++-22 prog.cpp -o prog_cpp 2>&1 \
           && ./prog_c 2>&1 \
           && ./prog_cpp 2>&1 \
           && make clean 2>&1 \
           && make prog_c prog_cpp 2>&1"
