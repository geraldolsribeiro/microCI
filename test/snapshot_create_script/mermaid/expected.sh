...
# ----------------------------------------------------------------------
function step_mermaid() {
  local SECONDS=0
  local MICROCI_STEP_NAME="Mermaid"
  local MICROCI_STEP_DESCRIPTION="Create diagram using mermaid"
...

  {
    (
      ...
      docker run \
        ...
        --attach stderr \
        --rm \
        --name microci_mermaid_$(head -c 8 /proc/sys/kernel/random/uuid) \
        --workdir /microci_workspace \
        --volume "${MICROCI_PWD}":/microci_workspace \
        --network host \
        intmain/microci_mermaid:latest \
        --input diagram.mmd \
        --output ./diagram.png 2>&1
...
if microCI_should_pull_docker_image 'intmain/microci_mermaid:latest'; then
  echo 'Updating intmain/microci_mermaid:latest docker image...'
  docker pull intmain/microci_mermaid:latest --quiet
fi
