...
# ----------------------------------------------------------------------
function step_publish_to_another_git_repository() {
  local SECONDS=0
  local MICROCI_STEP_NAME="Publish to another git repository"
...

  {
    (
      ...
      echo ""
      echo ""
      echo "Step: Publish to another git repository"
      ...
      docker run \
        ...
        --attach stderr \
        --rm \
        --name microci_publish_to_another_git_repository_$(head -c 8 /proc/sys/kernel/random/uuid) \
        --network host \
        ...
        /bin/bash -c "cd /microci_workspace \
           && git clone --branch gh-pages 'git@seuservidor.com.br:awesome_deploy.git' --depth 1 '/tmp/microci_deploy' 2>&1 \
           ...
           && git -C /tmp/microci_deploy push origin gh-pages 2>&1 \
...
function main() {
  ...
  step_publish_to_another_git_repository
