...
function step_manage_artifacts_in_bucket() {
  ...
  local HOME="/tmp/"
  local MICROCI_MINIO_ACCESS_KEY="xxxxx"
  local MICROCI_MINIO_SECRET_KEY="sssss"
  local MICROCI_MINIO_URL="http://10.20.30.40:5000"

  {
    (
      ...
      docker run \
        ...
        --env HOME="/tmp/" \
        --env MICROCI_MINIO_ACCESS_KEY="xxxxx" \
        --env MICROCI_MINIO_SECRET_KEY="sssss" \
        --env MICROCI_MINIO_URL="http://10.20.30.40:5000" \
        --volume "${MICROCI_PWD}":"/microci_workspace":rw \
        "minio/mc:latest" \
        /bin/bash -c "cd /microci_workspace \
        && mc alias set microci http://10.20.30.40:5000 xxxxx sssss --api S3v4 \
...
if microCI_should_pull_docker_image 'minio/mc:latest'; then
  echo 'Updating minio/mc:latest docker image...'
  docker pull minio/mc:latest --quiet
fi
