...

  {
    (
      ...
      docker run \
        ...
        --variable title='microCI' \
        --variable subtitle='Portable, Auditable Automation Pipelines' \
        --variable subject='microCI - Portable, Auditable Automation Pipelines' \
        --variable aspectratio=169 \
        --slide-level=2 \
        --to=beamer \
        \
        header-includes.yaml \
        01_the_ci_cd_problem.md \
...
if microCI_should_pull_docker_image 'pandoc/latex:latest'; then
  echo 'Updating pandoc/latex:latest docker image...'
  docker pull pandoc/latex:latest --quiet
fi
...
function main() {
  ...
  step_create_pdf_presentation_from_markdown
