...
for dir in */; do
  ...
  case "$test_name" in
    npm|beamer|docmd|doxygen|jfrog|minio)
      echo -e "[snapshot] ${YELLOW}SKIP${RESET}  $test_name"
      skip=$((skip + 1))
      continue
      ;;
  esac
  ...
  if "$script_dir/$dir/run.sh" >/dev/null; then
    echo -e "[snapshot] ${GREEN}PASS${RESET}  $test_name"
    pass=$((pass + 1))
  else
    echo -e "[snapshot] ${RED}FAIL${RESET}  $test_name"
    fail=$((fail + 1))
  fi
done

# Summary is intentionally compact for CI logs.
echo "[snapshot] summary  pass=$pass fail=$fail skip=$skip"
exit "$fail"
