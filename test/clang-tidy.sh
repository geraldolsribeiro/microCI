...
function step_gerar_relatorio_de_verificacao_do_codigo_c_____clang_tidy() {
  local SECONDS=0
  local MICROCI_STEP_NAME="Gerar relatório de verificação do código C++ - clang-tidy"
  local MICROCI_STEP_DESCRIPTION="Verifica o código C++ e gera relatório em formato HTML"
...

  {
    (
      ...
      echo ""
      echo ""
      echo "Step: Gerar relatório de verificação do código C++ - clang-tidy"
      ...
      docker run \
        ...
        --attach stderr \
        --rm \
        --name microci_gerar_relatorio_de_verificacao_do_codigo_c_____clang_tidy \
        --network none \
